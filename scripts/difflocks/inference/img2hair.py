...
class DiffLocksInference():
    def __init__(self, path_ckpt_strandcodec, path_config_difflocks, path_ckpt_difflocks, path_ckpt_rgb2material=None, cfg_val=1.0, nr_iters_denoise=100, nr_chunks_decode=50):
        ...
        self.dinov2_latents_model.to(device)
        ...
        self.strand_codec = StrandCodec(do_vae=False, 
                        ...
                        dim_per_value_decoded=3).to(device)
        self.strand_codec.load_state_dict(torch.load(path_ckpt_strandcodec, map_location='cpu'))
        ...
        inner_model_ema = K.config.make_model(config).to(device)
        ...
        if path_ckpt_rgb2material is not None:
            ...
            self.rgb2material.load_state_dict(torch.load(path_ckpt_rgb2material, map_location='cpu'))
    ...
    def rgb2hair(self, rgb_img, out_path=None):
        ...
        # 1) Run MediaPipe to detect and crop the face region
        ...
        if face_landmarks is None:
            # No face detected → nothing to do
            return None

        frame = crop_face(frame, face_landmarks, output_size=770)

        # 2) Convert back to a normalized tensor (BCHW, float)
        img_tensor = torch.tensor(frame).to(device)
        ...
        # 3) Compute DINOv2 latents (patch + CLS tokens)
        rgb_input = self.dinov2_latents_preprocessor(rgb_img).to(device)
        dinov2_output = self.dinov2_latents_model.forward_features(rgb_input)

        patch_tok = dinov2_output["x_norm_patchtokens"].clone()   # (B, num_patches, hidden_dim)
        cls_tok   = dinov2_output["x_norm_clstoken"].clone()      # (B, hidden_dim)

        # Reshape patch tokens to (B, hidden_dim, H, W)
        batch_size, num_patches, hidden_size = patch_tok.shape
        h = w = int(num_patches ** 0.5)  # e.g. 14 × 14 = 196 patches
        patch_embeddings_reshaped = patch_tok.reshape(batch_size, h, w, hidden_size)
        patch_embeddings_reshaped = patch_embeddings_reshaped.permute(0, 3, 1, 2).contiguous()
        ...
        # 4) Run the diffusion model to get a scalp‐texture (with density channel)
        is_cuda = (device.type == "cuda")

        if is_cuda:
            # On GPU, autocast to BFloat16 for FlashAttention
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                scalp_texture = sample_images_cfg(
                    1,
            ...
            # On CPU, just call normally (no autocast)
            scalp_texture = sample_images_cfg(
                1,
                cfg_val=self.cfg_val,
                cfg_interval=[0.0, 5.0],
        ...
        scalp_texture_orig = scalp_texture.clone()

        if out_path:
            np.savez(
                os.path.join(out_path, "scalp_texture.npz"),
                scalp_texture=scalp_texture_orig.cpu().numpy()
            )

        print("scalp_texture", scalp_texture.shape)

        # 5) Separate out the actual 64‐dim latent channels (drop the density channel)
        scalp_texture = scalp_texture_orig[:, 0:-1, :, :]   # (B, 64, 256, 256)

        # 6) Build the density map from the last channel
        density_map = scalp_texture_orig[:, -1:, :, :]  # (B,1,256,256)
        density_map = density_map * (0.5 / self.model_config["sigma_data"]) + 0.5
        density_map = density_map.clamp(0, 1)
        density_map[density_map < 0.02] = 0.0    # zero‐out low‐density regions

        # 7) Sample actual 3D strands from the scalp texture + density map
        strand_points_world, strand_points_tbn = sample_strands_from_scalp_with_density(
            ...
            nr_chunks=self.nr_chunks_decode_strands,
            upsample_multiplier=3
        )

        # 8) (Optional) Run the RGB → material model if requested
        ...
        if self.rgb2material is not None:
            ...
            if out_path and (hair_material_dict is not None):
                data = {
        ...
        # 9) Save the final 3D positions (world‐space) to NPZ
        if out_path:
        ...
        # 10) Save the cropped RGB face image
        if out_path:
    ...
    def file2hair(self, file_path, out_path):
        ...
        rgb_img = torch.tensor(frame).to(device)
