...
# ─────────────────────────────────────────────────────────────────
# 1) Attempt to load NATTEN (NeighborhoodAttention). If missing, set natten=None
# ─────────────────────────────────────────────────────────────────
try:
    import natten
    # If the library supports “use_kv_parallelism_in_fused_na”, call it; otherwise ignore.
    try:
...
# ─────────────────────────────────────────────────────────────────
# 2) Attempt to load FlashAttention-2. If missing, set flash_attn=None
# ─────────────────────────────────────────────────────────────────
try:
    import flash_attn
    _HAS_FLASH = True
...
# Add this modification to your NeighborhoodSelfAttentionBlock class
# Replace the existing forward method with this version:
...
class NeighborhoodSelfAttentionBlock(nn.Module):
    ...
    # Replace the NeighborhoodSelfAttentionBlock forward method with this debug version:
    ...
    def forward(self, x, pos, cond):
        ...
        # Check if NATTEN is available - FALLBACK ONLY when natten is None
        if natten is None:
            # FALLBACK: Use global attention when NATTEN is not available
            print("Warning: NATTEN not available, falling back to global attention")
            q, k, v = rearrange(qkv, "n h w (t nh e) -> t n nh (h w) e", t=3, e=self.d_head)
            q, k = scale_for_cosine_sim(q, k, self.scale[:, None, None], 1e-6)
            
            # Process position embeddings properly for global attention
            pos_flat = rearrange(pos, "... h w e -> ... (h w) e").to(qkv.dtype)
            theta = self.pos_emb(pos_flat)
            theta = theta.movedim(-2, -3)  # Move heads dimension to match q, k
            ...
            # ORIGINAL NATTEN CODE - unchanged
