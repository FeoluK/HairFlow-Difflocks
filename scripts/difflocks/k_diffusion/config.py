...
def load_config(path_or_dict):
    ...
    defaults_image_transformer_v2_cond_keyu = {
        'model': {
            'mapping_width': 256,
            'mapping_depth': 2,
            'mapping_d_ff': None,
    ...
    defaults_image_transformer_v2_conditional = {
        'model': {
            'mapping_width': 256,
            'mapping_depth': 2,
            'mapping_d_ff': None,
    ...
    elif config['model']['type'] == 'image_transformer_v2' or config['model']['type'] == 'image_transformer_v2_cond_keyu':
    ...
    elif config['model']['type'] == 'image_transformer_v2_conditional' or config['model']['type'] == 'image_transformer_v2_conditional_direct_map':
        config = merge(defaults_image_transformer_v2_conditional, config)
        if not config['model']['mapping_d_ff']:
            config['model']['mapping_d_ff'] = config['model']['mapping_width'] * 3
        if not config['model']['d_ffs']:
            d_ffs = []
...
def make_model(config):
    ...
    if config['type'] == 'image_v1':
        ...
        for depth, width, d_ff, self_attn, dropout in zip(config['depths'], config['widths'], config['d_ffs'], config['self_attns'], config['dropout_rate']):
            if self_attn['type'] == 'global':
                self_attn = models.image_transformer_v2_conditional.GlobalAttentionSpec(self_attn.get('d_head', 64))
            elif self_attn['type'] == 'neighborhood':
                self_attn = models.image_transformer_v2_conditional.NeighborhoodAttentionSpec(self_attn.get('d_head', 64), self_attn.get('kernel_size', 7))
            elif self_attn['type'] == 'shifted-window':
                self_attn = models.image_transformer_v2_conditional.ShiftedWindowAttentionSpec(self_attn.get('d_head', 64), self_attn['window_size'])
            elif self_attn['type'] == 'none':
                self_attn = models.image_transformer_v2_conditional.NoAttentionSpec()
            else:
                raise ValueError(f'unsupported self attention type {self_attn["type"]}')
            levels.append(models.image_transformer_v2_conditional.LevelSpec(depth, width, d_ff, self_attn, dropout))
        mapping = models.image_transformer_v2_conditional.MappingSpec(config['mapping_depth'], config['mapping_width'], config['mapping_d_ff'], config['mapping_dropout_rate'])
    ...
    elif config['type'] == 'image_transformer_v2_conditional':
        assert len(config['widths']) == len(config['depths'])
        assert len(config['widths']) == len(config['d_ffs'])
        assert len(config['widths']) == len(config['self_attns'])
        assert len(config['widths']) == len(config['dropout_rate'])
        ...
        for depth, width, d_ff, self_attn, dropout in zip(config['depths'], config['widths'], config['d_ffs'],
            ...
            if self_attn['type'] == 'global':
                ...
                self_attn = models.image_transformer_v2_conditional.NeighborhoodAttentionSpec(
                    self_attn.get('d_head', 64), self_attn.get('kernel_size', 7))
            elif self_attn['type'] == 'shifted-window':
                self_attn = models.image_transformer_v2_conditional.ShiftedWindowAttentionSpec(
                ...
                raise ValueError(f'unsupported self attention type {self_attn["type"]}')
            levels.append(models.image_transformer_v2_conditional.LevelSpec(depth, width, d_ff, self_attn, dropout))
        mapping = models.image_transformer_v2_conditional.MappingSpec(config['mapping_depth'], config['mapping_width'],
                                                                      config['mapping_d_ff'],
                                                                      config['mapping_dropout_rate'])
        model = models.ImageTransformerDenoiserModelV2Conditional(
            levels=levels,
            mapping=mapping,
            in_channels=config['input_channels'],
            out_channels=config['input_channels'],
            ...
            num_classes=num_classes + 1 if num_classes else 0,
            mapping_cond_dim=config['mapping_cond_dim'],
            condition_dropout_rate= config['condition_dropout_rate'],
            # rgb_model_condig=config['rgb_model']
            rgb_condition_config = config['rgb_condition']
        )
    elif config['type'] == 'image_transformer_v2_conditional_direct_map':
        assert len(config['widths']) == len(config['depths'])
        assert len(config['widths']) == len(config['d_ffs'])
        assert len(config['widths']) == len(config['self_attns'])
        assert len(config['widths']) == len(config['dropout_rate'])
        ...
        for depth, width, d_ff, self_attn, dropout in zip(config['depths'], config['widths'], config['d_ffs'],
                                                          config['self_attns'], config['dropout_rate']):
            if self_attn['type'] == 'global':
                self_attn = models.image_transformer_v2_conditional_direct_map.GlobalAttentionSpec(self_attn.get('d_head', 64))
            elif self_attn['type'] == 'neighborhood':
                self_attn = models.image_transformer_v2_conditional_direct_map.NeighborhoodAttentionSpec(
                    self_attn.get('d_head', 64), self_attn.get('kernel_size', 7))
            elif self_attn['type'] == 'shifted-window':
                self_attn = models.image_transformer_v2_conditional_direct_map.ShiftedWindowAttentionSpec(
                    self_attn.get('d_head', 64), self_attn['window_size'])
            elif self_attn['type'] == 'none':
                self_attn = models.image_transformer_v2_conditional_direct_map.NoAttentionSpec()
            else:
                raise ValueError(f'unsupported self attention type {self_attn["type"]}')
            levels.append(models.image_transformer_v2.LevelSpec(depth, width, d_ff, self_attn, dropout))
        mapping = models.image_transformer_v2_conditional_direct_map.MappingSpec(config['mapping_depth'], config['mapping_width'],
                                                                      config['mapping_d_ff'],
                                                                      config['mapping_dropout_rate'])
        model = models.ImageTransformerDenoiserModelV2ConditionalDirectMap(
            levels=levels,
            mapping=mapping,
            in_channels=config['input_channels'],
            out_channels=config['input_channels'],
    ...
    elif config['type'] == 'image_transformer_v2_cond_keyu':
        ...
        for depth, width, d_ff, self_attn, dropout in zip(config['depths'], config['widths'], config['d_ffs'], config['self_attns'], config['dropout_rate']):
            if self_attn['type'] == 'global':
                self_attn = models.image_transformer_v2_cond_keyu.GlobalAttentionSpec(self_attn.get('d_head', 64))
            elif self_attn['type'] == 'neighborhood':
                self_attn = models.image_transformer_v2_cond_keyu.NeighborhoodAttentionSpec(self_attn.get('d_head', 64), self_attn.get('kernel_size', 7))
            elif self_attn['type'] == 'shifted-window':
                self_attn = models.image_transformer_v2_cond_keyu.ShiftedWindowAttentionSpec(self_attn.get('d_head', 64), self_attn['window_size'])
            elif self_attn['type'] == 'none':
                self_attn = models.image_transformer_v2_cond_keyu.NoAttentionSpec()
            else:
                raise ValueError(f'unsupported self attention type {self_attn["type"]}')
            levels.append(models.image_transformer_v2_cond_keyu.LevelSpec(depth, width, d_ff, self_attn, dropout))
        mapping = models.image_transformer_v2_cond_keyu.MappingSpec(config['mapping_depth'], config['mapping_width'], config['mapping_d_ff'], config['mapping_dropout_rate'])
        model = models.ImageTransformerDenoiserModelV2Conditional_keyu(
            levels=levels,
            mapping=mapping,
            in_channels=config['input_channels'],
            out_channels=config['input_channels'],
            ...
            num_classes=num_classes + 1 if num_classes else 0,
            mapping_cond_dim=config['mapping_cond_dim'],
            use_cross_cond= bool(config['use_cross_cond']),
            cross_attns_config=config['cross_attns'],
            fine_tune_clip= bool(config['clip']['fine_tune_clip'])
        )
    else:
...
def make_denoiser_wrapper(config):
    ...
    parametrization = config.get('parametrization', 'v')
    loss_weight_per_channel = config.get('loss_weight_per_channel', None)
    if loss_config == 'karras':
        weighting = config.get('loss_weighting', 'karras')
        scales = config.get('loss_scales', 1)
        if not has_variance:
            return partial(layers.Denoiser, sigma_data=sigma_data, weighting=weighting, scales=scales, parametrization=parametrization, loss_weight_per_channel=loss_weight_per_channel)
