...
def prepare_gt_batch(batch):
    ...
    gt_strand_positions, gt_root_normals = world_to_tbn_space(tbn, 
                                                              positions, 
                                                              root_normal)
    gt_strand_positions=gt_strand_positions.to(device)
...
def generate_scalp_textures(batch, model, normalization_dict, tex_size, output_scalp_texture_path):
    ...
    strand_uv=strand_uv.to(device).squeeze(0)
    ...
    scalp_texture = torch.zeros(1,64,tex_size,tex_size).to(device)
    scalp_texture[:,:,strand_indices[:,1],strand_indices[:,0]] = latents.transpose(0,1).unsqueeze(0) #make latents 1,64,nr_strands

    #get mask
    scalp_mask = torch.zeros(1,1,tex_size,tex_size).to(device)
    homogeneous_coord = torch.ones(nr_strands,1).to(device)
...
def horizontally_flip(batch, scalp_mesh_data):
    ...
    #put it back on cuda
    tbn=torch.from_numpy(tbn).unsqueeze(0).to(device)
    positions=torch.from_numpy(positions).unsqueeze(0).to(device)
    root_normal=root_normal.unsqueeze(0).to(device)
